...
def get_activations(il_placements, transactions, season_dates):
    off_il_mask = ((((transactions['description'].str.contains("activated|reinsated|recalled|returned|status changed")) | (transactions['typecode'].isin(['DFA','REL','RET','OUT','DEI','DEC','OPT', 'DES', 'CLW']))) | 
    ((transactions.typecode == 'ASG') & transactions.toteam_id.isin(mlb_teams['id']))) & ~transactions['description'].str.contains("all-stars"))
    ...
    injury = pd.merge_asof(il_placements, off_il, by='person_id' ,left_on='il_place_date', right_on='return_date',
    suffixes=('', '_return'), direction='forward', allow_exact_matches=False)
    injury = injury.rename(columns={'toteam_id':'return_team_id','trans_id':'return_trans_id'})
    injury['season'] = injury.il_place_date.dt.year
    injury = pd.merge(injury, season_dates.groupby('season').last_game.max(), left_on=['season'], right_on=['season'])
    injury['last_game'] = injury['last_game'].dt.date
    offseason_return_mask = (injury.return_date > injury.last_game + timedelta(7)) | ((injury.return_date.isnull()) & (~injury.last_game.isnull()))
...
def check_for_player_app(injury, player_app):
    ...
    injury = pd.merge_asof(injury, player_app, by='person_id', left_on='il_place_date', right_on='game_date',
    suffixes=('', '_app'), direction='forward', allow_exact_matches=False)
    return_before_trans_mask = injury.game_date < injury.return_date
    injury.loc[return_before_trans_mask, 'return_team_id'] = injury.loc[return_before_trans_mask, 'team_id']
    injury.loc[return_before_trans_mask, 'return_trans_id'] = None
    injury.loc[return_before_trans_mask, 'return_date'] = injury.loc[return_before_trans_mask, 'game_date']
...
def bridge_il_stints(injury, season_dates):
    ...
    injury['season'] = injury['il_place_date'].dt.year
    injury = injury.merge(season_dates[['season', 'last_game','team_id']], how='left', left_on=['season','return_team_id'], right_on=['season','team_id'])
    #injury = injury.drop(['team_id_x'],axis=1)
    injury['next_season'] = injury['season'] + 1
    injury = injury.merge(season_dates[['season','first_game','team_id']], how='left', \
    left_on=['next_season','next_il_team'], right_on=['season','team_id'])
    injury = injury.drop(['season_x','next_season','season_y'],axis=1)
    injury = injury.rename(columns={'first_game':'next_season_first_game'})
    injury['offseason_activation'] = injury.return_date.between(injury.last_game, pd.to_datetime(pd.Series(injury.last_game.dt.year.astype('Int64'), dtype='string') + '-12-31'))
    injury['pre_od_placement'] = (injury.next_il_placement <= injury.next_season_first_game) & (injury.next_il_placement.dt.year == injury.next_season_first_game.dt.year)
    cu_unknown = injury['body_part_group'].isna() | (injury['body_part_group'] == 'undisclosed')
    nx_unknown = injury['next_body_part_group'].isna() | (injury['next_body_part_group'] == 'undisclosed')
    ...
    sev_mask = (
    injury['injury_type'].str.contains('surgery', case=False, na=False) |
    injury['injury_type'].str.contains('tear', case=False, na=False)   # catches 'tear' and 'partial tear'
    )

    mask = (
        (((injury['return_date'].isnull()) & ~(injury['next_il_placement'].isnull()) |
    (injury['offseason_activation'] & injury['pre_od_placement'])))
    & (
        ...
        | (cu_unknown & ~nx_unknown & (injury['prev_known_bpg'] == injury['next_bpg_clean']))
    ...
    on=['person_id','bridge_group'], how='left'
    )
    injury[['chain_team_return','chain_date_return','chain_trans_id_return', 'chain_body_part','chain_injury_type', 'chain_raw_injury','chain_body_part_group']] = (
    injury.groupby(['person_id','bridge_group'])[['return_team_id','return_date','return_trans_id', 'body_part','injury_type', 'raw_injury', 'body_part_group']]
          .transform('last', skipna=False))
    # Final picks for "second_*": prefer surgery/tear if present, else fall back to existing chain_*
    injury['final_second_body_part']  = injury['chain_body_part'].where(injury['sev_body_part'].isna(),  injury['sev_body_part'])
    injury['final_second_injury_type'] = injury['chain_injury_type'].where(injury['sev_injury_type'].isna(), injury['sev_injury_type'])
    injury['final_second_raw_injury']  = injury['chain_raw_injury'].where(injury['sev_raw_injury'].isna(),  injury['sev_raw_injury'])
    needs_bridge = injury.groupby(['person_id','bridge_group'])['bridge'].transform('max').astype(bool)
    tgt_cols = ['team_return','return_date','return_trans_id','second_body_part','second_injury_type','second_raw_injury']
    injury.loc[needs_bridge, tgt_cols] = injury.loc[
        needs_bridge,
        ['chain_team_return','chain_date_return','chain_trans_id_return',
        'final_second_body_part','final_second_injury_type','final_second_raw_injury']
    ].to_numpy()
    keep_first = injury.groupby(['person_id','bridge_group']).cumcount() == 0
    injury = injury[keep_first]
    injury.loc[injury.body_part_group.isnull(), 'body_part_group'] = injury.loc[injury.body_part_group.isnull(), 'chain_body_part_group']
    injury = injury.drop(columns=['bridge','bridge_break','bridge_group','offseason_activation', 'pre_od_placement','chain_team_return','chain_date_return','chain_trans_id_return','bpg_clean','next_bpg_clean','prev_known_bpg',
                         'sev_body_part','sev_injury_type','sev_raw_injury',
    'final_second_body_part','final_second_injury_type','final_second_raw_injury'])
    new_columns = {'il_place_trans_id':'il_place_trans_id', 'il_place_date':'il_place_date', 'person_id':'person_id','il_place_team':'il_place_team', 'raw_injury': 'raw_injury',
                   'side':'side', 'body_part':'body_part','injury_type':'injury_type','body_part_group':'body_part_group' ,'second_raw_injury':'second_raw_injury',
               'second_body_part': 'second_body_part', 'second_injury_type':'second_injury_type', 'return_trans_id':'return_trans_id','return_date':'return_date','return_team_id':'return_team'}
    injury = injury.rename(columns=new_columns)
    injury = injury[list(new_columns.values())]
    current_date = pd.Timestamp.now().normalize()
    injury['injury_length'] = (injury.return_date.fillna(current_date) - injury.il_place_date)
...
def create_silver_injury_spans(il_placements, transactions, season_dates, engine):
    ...
    injury.to_sql('injury_spans', engine, schema='silver', index=False, if_exists='replace')

"""
Bronze-layer ingestion for MLB schedule/games from statsapi.mlb.com.


- Fetches games by date range
- Normalizes the JSON into a flat DataFrame
- Ensures bronze tables exist (target + staging) and upserts via MERGE
"""
import requests
...
def get_games_data(start_date: str, end_date: str, primary_keys: list):
    """Retrieve and normalize MLB games between two dates (inclusive).


    Args:
    start_date: ISO date string (YYYY-MM-DD).
    end_date: ISO date string (YYYY-MM-DD).
    primary_keys: Columns to de-duplicate on (e.g., ['gamepk']).


    Returns:
    pd.DataFrame: One row per game (flattened), de-duplicated, with all-null columns removed.
    """
    ...
    for date in dates:
        ...
        games_df = games_df.drop_duplicates(subset=primary_keys, keep='last')
        games_df = games_df.dropna(subset=primary_keys, axis=0)
        games_df = games_df.dropna(axis=1, how='all')
    return games_df

    

def create_games(start_date: datetime.date, end_date:datetime.date, chunk_size:int, \
                    engine, base: str, schema: str, primary_keys=None):
    """Ingest MLB games into bronze in small date chunks.

    On the first chunk, infers a basic schema and ensures tables exist. Thereafter,
    reconciles schema drift and uses COPY -> MERGE to upsert.
    """
    first_chunk = True
    for year in range(start_date.year, end_date.year + 1):
        ...
        for chunk_start, chunk_end in tqdm(date_chunks, desc=f"fetching {year}"):
            try:
                df = get_games_data(start_date=chunk_start, end_date=chunk_end, primary_keys=primary_key)
                if df.empty:
                    continue
                if first_chunk:
...
if __name__ == "__main__":
    ...
    start_date = get_start_date(engine=engine, base=base_table_name, column_name='gamedate',schema=schema, window=7)
    ...
    try:
        ...
        print(e)
        exit()
    create_games(start_date=start_date, end_date=end_date, chunk_size=chunk_size,\

...
from create_il_movement import setup_injury_parser, get_il_data
from datetime import date, timedelta
from sqlalchemy import create_engine, text
import os


def update_il_mov(engine, today, chunk_size, table_name):
    nlp, parse_injury_text = setup_injury_parser()
    start_date = get_start_date(engine=engine, column_name='date',table_name='il_mov')
    end_date = today - timedelta(days=1)
    if end_date - start_date > timedelta(days= chunk_size):
        chunks =  get_date_chunks(start_date, end_date, chunk_size)
        for start, end in chunks:
            df = get_il_data(start, end, parse_injury_text)
            try:
        ...
        df = get_il_data(start_date, end_date, parse_injury_text)
        try:
            fast_copy_from(df, table_name, engine)
...
if __name__ == "__main__":
    ...
    try:
        ...
        exit()
    # Run the update
    update_il_mov(engine, today, chunk_size, table_name)
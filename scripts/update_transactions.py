...
import os
from create_transactions import get_transactions_data

def update_transactions(engine, today, chunk_size, table_name):
    start_date = get_start_date(engine=engine, column_name='date',table_name=table_name)
    end_date = today - timedelta(days=1)
    if end_date - start_date > timedelta(days= chunk_size):
        chunks =  get_date_chunks(start_date, end_date, chunk_size)
        for start, end in chunks:
            df = get_transactions_data(start, end)
            try:
        ...
        df = get_transactions_data(start_date, end_date)
        try:
            fast_copy_from(df, table_name, engine)
...
if __name__ == "__main__":
    ...
    try:
        ...
        exit()
    # Run the update
    update_transactions(engine, today, chunk_size, table_name)